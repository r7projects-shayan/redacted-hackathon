                ...
                <StreakContainer >
                    ...
                    <Grid >
                        {fullYear.map((day, index)=>(
                            ...
                            <Day key={index} title={`${day} , ${data.find(item=>(item.date === day))?.done === 1 ? data.find(item=>(item.date === day))?.done + " contribution" : data.find(item=>(item.date === day))?.done + " contributions"} `} color={getColor(data.find(item=>(item.date === day))?.done)}></Day>
                        ))}
                            {/*// </div>*/}
